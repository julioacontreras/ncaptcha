...
class NCaptcha {
  constructor(params) {
    ...
    params.numberOfLines = params.numberOfLines || 2;
  ...
  generate() {
    ...
    const canvas = createCanvas(this.params.canvasWidth, this.params.canvasHeight);
    ...
    for (var i = 0; i < this.params.numberOfLines; i++) {
      ctx.moveTo(Math.floor(0.08 * this.params.canvasWidth), Math.random() * this.params.canvasHeight);
      ctx.bezierCurveTo(
        Math.floor(0.32 * this.params.canvasWidth),
        Math.random() * this.params.canvasHeight,
        Math.floor(1.07 * this.params.canvasHeight),
        Math.random() * this.params.canvasHeight,
        Math.floor(0.92 * this.params.canvasWidth),
        Math.random() * this.params.canvasHeight
      );
      ctx.stroke();
    }
